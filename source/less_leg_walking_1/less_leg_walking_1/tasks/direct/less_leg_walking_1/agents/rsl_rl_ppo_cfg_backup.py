...
class LessLegWalkingFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        clip_param=0.2,
        entropy_coef=0.01,  # tighter exploration after MoE change
        num_learning_epochs=8,  # More learning epochs
        num_mini_batches=4,
        learning_rate=2.0e-4,  # smaller lr for stability with MoE
        schedule="adaptive",
        gamma=0.99,
        lam=0.95,
        desired_kl=0.01,
...
class LessLegWalkingRoughPPORunnerCfg(LessLegWalkingFlatPPORunnerCfg):
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        use_clipped_value_loss=True,
        clip_param=0.2,
        entropy_coef=0.0025,  # damp exploration for rough terrain
