...
@configclass
class CompoCfg(RslRlPpoActorCriticCfg):
    """Configuration for the CompNet policy."""
    # raw_obs_dim: int = 226
    # hidden_dim_moe: list[int] = [512, 256, 128]
    obs_dim: int = 235
    device: str = "cuda"
    class_name: str = "CompoActorCritic"
    activation: str = "elu"
...
class CompoActorCritic(ActorCritic):
    def __init__(self, obs, obs_groups, num_actions, **kwargs):
        ...
        internal_hidden_dims = kwargs.pop('internal_policy_hidden_dims')
        ...
        hidden_dim = internal_hidden_dims[0]
        obs_dim = kwargs.pop("obs_dim")
        prevs_dir = kwargs.pop("prevs_dir")
        ...
        super().__init__(
            ...
            actor_hidden_dims=actor_hidden_dims, # Placeholder arg, this is not for the real actor, which is a CompoNet
        ...
        def internal_policy_gen():
            if len(prevs_dir) > 0:
                input_dim_internal = obs_dim + hidden_dim
            else:
        ...
        if len(prevs_dir) > 0:
            prevs = []
            for pre_dir in prevs_dir:
                print("Loading prevs..............")
                complete_model = torch.load(pre_dir, map_location=self.device, weights_only=False)
                unit = complete_model["actor"]
                print(type(unit))
                prevs.append(unit)
            ...
            first_module = internal_policy_gen()
            self.actor = FirstModuleWrapper(first_module, ret_probs=False)
            

        # Note: CompoNet is currently using ret_probs=False for continuous action spaces. 
        # By default in the original script it used ret_probs=True, but we're predicting actions, so False.
        
        


    def forward(self, obs):
        action, phi = self.actor(obs)
